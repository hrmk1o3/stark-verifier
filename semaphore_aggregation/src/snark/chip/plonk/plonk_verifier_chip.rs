...
impl PlonkVerifierChip<F> {
    ...
    pub fn get_challenges(
        ...
        let plonk_betas = transcript_chip.squeeze(layouter, num_challenges)?;
        let plonk_gammas = transcript_chip.squeeze(layouter, num_challenges)?;
        ...
        let plonk_alphas = transcript_chip.squeeze(layouter, num_challenges)?;
        ...
        let plonk_zeta = transcript_chip.squeeze(layouter, 2)?;

        let fri_openings = openings.to_fri_openings();

        for v in fri_openings.batches {
            for ext in v.values {
                transcript_chip.write_extension(&ext)?;
            }
        }

        // Scaling factor to combine polynomials.
        let fri_alpha =
            AssignedExtensionFieldValue(transcript_chip.squeeze(layouter, 2)?.try_into().unwrap());

        // Recover the random betas used in the FRI reductions.
        let fri_betas = commit_phase_merkle_cap_values
            .iter()
            .map(|cap| {
                transcript_chip.write_cap(cap)?;
                let fri_beta = transcript_chip.squeeze(layouter, 2)?;
        ...
        transcript_chip.write_scalar(pow_witness)?;
        let fri_pow_response = transcript_chip.squeeze(layouter, 1)?[0].clone();

        let num_fri_queries = common_data.config.fri_config.num_query_rounds;
        let fri_query_indices = transcript_chip.squeeze(layouter, num_fri_queries)?;
    ...
    pub fn verify_proof_with_challenges(
        ...
        let (zeta_next, offset, merkle_caps) = layouter.assign_region(
            || "Verify proof",
            |region| {
                ...
                Ok((zeta_next, offset, merkle_caps))
            }
        )?;
        ...
        fri_chip.verify_fri_proof(
            layouter.namespace(|| "verify FRI proof"),
            &merkle_caps,
